...
The full [sports-skills](https://pypi.org/project/sports-skills/) CLI is
bundled with `machina-cli` and mounted dynamically under `machina sports`.
Every module and command that `sports-skills` exposes is delegated live —
there is no hand-written allow-list, so new sports-skills releases surface
automatically without a machina-cli upgrade.
