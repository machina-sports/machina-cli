...
### Fixed
- **Auto-heal no longer starves on undetermined bracket slots.** Live on staging, 3 of every 5 heal dispatches went to fixtures like `W95 vs W96` / `Brazil vs W78`: the research workflow drops undetermined fixtures by design (nothing to research until the teams are known), so they were re-dispatched every round and the backlog never drained (stuck at 18). `scan_edges` now hands the heal only team-determined live fixtures (`flagged[].fixture_ids`; the slots ride along as `undetermined_ids`), and groups made *entirely* of bracket slots — which share the "teams not yet known" text on purpose — are counted separately as `broken_placeholder_edges` / `placeholder_groups` instead of as misattributions (same treatment as already-played matches). Mixed groups stay broken: a real fixture carrying a placeholder's text is a live misattribution. The connector source now has a local test that exec's it against a fake `document_search`, the way the pod runs it.
